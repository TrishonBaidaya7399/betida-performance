...
const renderPayout = (row: CasinoBetData) => {
...
const renderBetAmount = (row: CasinoBetData) => {
...
const renderMultiplier = (row: CasinoBetData) => (
  <span className="text-foreground/55 font-medium">{row.multiplier}</span>
);
...
export default React.memo(function MyBetsCasinoTable({
  casinoBets,
}: {
  casinoBets: CasinoBetData[];
}) {
  const [currentPage, setCurrentPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedRow, setSelectedRow] = useState<CasinoBetData | null>(null);

  const totalPages = useMemo(
    () => Math.ceil(casinoBets.length / rowsPerPage),
    [casinoBets.length, rowsPerPage]
  );
  const currentData = useMemo(() => {
    const startIndex = (currentPage - 1) * rowsPerPage;
    const endIndex = startIndex + rowsPerPage;
    return casinoBets.slice(startIndex, endIndex);
  }, [casinoBets, currentPage, rowsPerPage]);
  ...
  const openBetDetailsModal = useCallback((row: CasinoBetData) => {
    setSelectedRow(row);
    setIsModalOpen(true);
  }, []);

  const desktopColumns: ColumnType<CasinoBetData>[] = [
    {
      key: "gameTitle",
      label: "Game",
      render: (row: CasinoBetData) => (
  ...
  const mobileColumns: ColumnType<CasinoBetData>[] = [
    {
      key: "gameTitle",
      label: "Game",
      render: (row: CasinoBetData) => (
    ...
    },
    {
      ...
      render: (row: CasinoBetData) => (
        <button
          ...
          className="font-medium text-left hover:underline text-foreground flex flex-row items-center gap-1"
        >
          <File size={20} className="text-foreground/55" />
          {row.betId}
        </button>
      ),
    },
  ...
  return (
    <div className="w-full">
      <div className="hidden md:block">
        <GlobalTable<CasinoBetData>
          columns={desktopColumns}
          data={currentData}
          loading={false}
          emptyMessage="No casino bets found."
          maxHeight={440}
        />
      </div>
