...
export default async function ProvablyFairConversions() {
  const data = await fetchProvablyFairByType("Conversions");
  return (
    <div className="bg-background-1 rounded-lg p-6 max-w-242 w-full mx-auto h-auto flex flex-col gap-6">
      {data?.length > 0 ? (
        ...
        <p className="text-foreground/60">No data found for Conversions.</p>
      )}
    </div>
  );
}
